class UomsController < ApplicationController
  ...
  def update
    respond_to do |format|
      if @uom.update(uom_params)
        format.html { redirect_to @uom, notice: "Uom was successfully updated.", status: :see_other }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to uoms_path, notice: "Uom was successfully destroyed.", status: :see_other }
  ...
  private
    ...
    def uom_params
      params.require(:uom).permit(:name, :quantity)
    end
end
